...
Persistence:
  ...
  - candle_log.csv          : every closed 1m candle (bar buffer state)
...
class PaperTradeJournal:
    def __init__(self):
        # Restore capital from previous session or start fresh
        ...
        # Closed trades this session (already persisted in CSV)
        self.closed_trades = []
    ...
    def open_trade(self, side, entry, sl, tp, ml_label, ml_conf,
        ...
        # Log the open event to signal log
    ...
    def update_positions(self, live_price, ts):
        ...
        for t in self.open_pos:
            ...
            if hit_sl or hit_tp:
                ...
                # Persist closed trade to CSV immediately
    ...
    def compute_stats(self):
        # Load ALL closed trades from CSV (includes previous sessions)
...
class RenkoMLPaperTrader:
    ...
    def on_open(self, ws):
        ...
        if len(self.buffer) >= 30:
            ...
            print("[WS] Waiting for closed 1m candles...")
        print("")
    ...
    def on_message(self, ws, message):
        try:
            ...
            with self._lock:
                ...
                # Log every closed candle to candle_log.csv
    ...
    def _process(self):
        ...
        # Step 3: No new brick = no signal possible
        if new_bricks <= 0:
        ...
        # Step 4: Renko signal check — if HOLD, stop here
        if renko_signal == 0:
            return
        ...
        # Log every signal event (fired or skipped) to signal_log.csv
        append_csv(SIGNAL_LOG_CSV, {
    ...
    def run(self):
        print("=" * 70)
        print("  SCRIPT 3: RENKO + ML PAPER TRADING")
